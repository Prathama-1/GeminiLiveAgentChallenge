...
function initStockfish() {
  try {
    // stockfish.js loaded as global — create a Web Worker from it
    if (typeof Stockfish === 'undefined') {
      // Try loading as worker directly
...
function setDifficulty(level) {
  ...
  // Update labels
  const name = DIFF_NAMES[level];
  const labelEl = document.getElementById('diff-current-label');
  const badgeEl = document.getElementById('diff-badge-label');
  ...
  setMessage(`Difficulty set to ${name}.`);
  speak(`Difficulty set to ${name}.`);
}
...
// ─── BLACK ENGINE ────────────────────────────────────────────
function blackMove() {
  if (difficulty === 'random' || !stockfish || !sfReady) {
    blackMoveRandom();
    return;
  }
  ...
  getStockfishMove(fen, depth, (uciMove) => {
    ...
    if (!result) { blackMoveRandom(); return; }
